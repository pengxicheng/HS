...
- (void)viewDidLoad {
    ...
    if ([UIApplication sharedApplication].keyWindow.rootViewController == self.navigationController || [UIApplication sharedApplication].keyWindow.rootViewController == self) {
...
//    UMComPageControlView *titlePageControl = [[UMComPageControlView alloc]initWithFrame:CGRectMake(0, 0, 180, 25) itemTitles:[NSArray arrayWithObjects:UMComLocalizedString(@"focus", @"关注"),UMComLocalizedString(@"recommend",@"推荐"),UMComLocalizedString(@"topic",@"话题"), nil] currentPage:0];
        UMComPageControlView *titlePageControl = [[UMComPageControlView alloc]initWithFrame:CGRectMake(0, 0, 180, 25) itemTitles:[NSArray arrayWithObjects:UMComLocalizedString(@"focus", @"关注"),UMComLocalizedString(@"topic",@"话题"), nil] currentPage:0];
...
//    [titlePageControl setItemImages:[NSArray arrayWithObjects:UMComImageWithImageName(@"left_frame"),UMComImageWithImageName(@"midle_frame"),UMComImageWithImageName(@"right_item"), nil]];
        [titlePageControl setItemImages:[NSArray arrayWithObjects:UMComImageWithImageName(@"left_frame"),UMComImageWithImageName(@"right_item"), nil]];
...
//            weakSelf.findButton.hidden = YES;//隐藏发现按钮
            ...
            findViewController.hidesBottomBarWhenPushed = YES;
            //隐藏编辑按钮
            
            UMComFeedTableViewController *focusedTableController = weakSelf.childViewControllers[0];
            focusedTableController.editButton.hidden = YES;
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText
{
    if (self.titlePageControl.currentPage == 1) {
        if (searchBar.text.length == 0) {
            UMComTopicsTableViewController *topicTableVc = self.childViewControllers[1];
            topicTableVc.fetchRequest = [[UMComAllTopicsRequest alloc] initWithCount:BatchSize];
            [topicTableVc loadAllData:nil fromServer:nil];
        }
    }
}
...
- (BOOL)searchBarShouldBeginEditing:(UISearchBar *)searchBar
{
    __weak typeof(self) weakSelf = self;
    if (self.titlePageControl.currentPage != 1) {
        [[UMComAction action] performActionAfterLogin:searchBar.text viewController:self completion:^(id responseObject, NSError *error) {
            if (!error) {
